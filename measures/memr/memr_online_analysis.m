function [result, done] = memr_online_analysis(trials_so_far, stim_info, params)
%
% Called after each trial with the full accumulated trials matrix so far.

%% --- Setup ---

% Determine if it's a swept or discrete run:
switch lower(params.stim_type)

    case 'swept'
        
        elicitor = stim_info.stim_noise(:,1);
        Clicks = stim_info.stim_click(:,1);

        noiseLvl = zeros(size(elicitor));
        noiseLvl(numel(elicitor)-numel(stim_info.h)+1:end,1) = 20*log10(stim_info.h./.00002) - stim_info.noiseatt;

        % get the stimulus levels ---------------------------------------------
        [rows,~] = size(elicitor);
        chunkSize = numel(stim_info.singleClick);
        totalClicks = stim_info.clicksPerTrain + params.n_baseline_clicks;

        noiseLvl = noiseLvl(1:chunkSize*totalClicks);
        Clicks = Clicks(1:chunkSize*totalClicks);
        time = stim_info.t(1,1:chunkSize*totalClicks);
        ...
        resp = trials_so_far(:,1:chunkSize*totalClicks);
        ...
        nSamples = ceil(params.click_win_dur_ms * 1e-3  * fs) +1; % total number of samples in just the click that we want to look at
        R = R(:,:,params.pad_samps+(1:nSamples)); % Get rid of the noise part, just have click;

        sampsToAnalyze = 700+(1:1024);

        temp_baseline = trimmean(R(:, 1:stim_info.n_baseline_clicks+1, sampsToAnalyze),20, 'floor', 1);
        ...
        for i = params.n_baseline_clicks+1+1:size(R, 2) % loop through each level skipping the first baseline clicks
            temp = trimmean(R(:,i, sampsToAnalyze), 20, 'floor', 1);
            temp = squeeze(temp);
            temp = detrend(temp);

            tempf = pmtm(temp, 4, freq, fs);
            temp_freq(:,i-(params.n_baseline_clicks+1)) = tempf; %median(tempf,2);
        end
        ...
        for k = 1:totalClicks-stim.n_baseline_clicks-1 % now look at clicks other than baseline clicks
            ...
            levels(1,k) = RMS(k+params.n_baseline_clicks);
            Ts(1,k) = T(1,k+params.n_baseline_clicks);
        end
        ...
        % Can set different if want some sort of stopping criterion
        done = TRUE; 
